...
async function run(opts) {
  ...
  // Synchronize .ralph/ralph-tasks.md symlink to the OpenSpec tasks.md so both
  // the loop engine and openspec-apply-change always operate on the same file.
  ...
  while (iterationCount < maxIterations) {
    ...
    // Auto-commit if not suppressed and there are file changes
    if (!options.noCommit && result.filesChanged && result.filesChanged.length > 0) {
      _autoCommit(iterationCount, result.filesChanged, options.verbose);
    }
...
/**
 * Auto-commit changed files after an iteration.
 * Runs `git add -A` and `git commit` with an auto-generated message.
 * Silently skips if git is unavailable or there is nothing to commit.
 *
 * @param {number} iteration     - Current iteration number
 * @param {Array<string>} files  - Files changed during this iteration
 * @param {boolean} verbose
 */
function _autoCommit(iteration, files, verbose) {
  try {
    ...
    const message = `mini-ralph auto-commit: iteration ${iteration} (${files.length} file(s) changed)`;
...
module.exports = { run, _containsPromise, _validateOptions, _autoCommit, _resolveStartIteration };
