...
async function runSupervisor(options = {}) {
  ...
  const resolveRunLogPaths = options.resolveRunLogPaths || (() => ({ stdoutLog: '', stderrLog: '' }));
  const detectSupervisorLogReads = options.detectSupervisorLogReads || (() => ({
  ...
  while (triesUsed < maxTries && totalAttempts < hardAttemptCap) {
    ...
    if (parsed.current_task_patch === null) {
      ...
      return _buildSupervisorReturn({
        ...
        blockerHash,
        hints: config.selfHealHints ? parsed.investigation_hints : [],
        hintsDropped: [],
        readLogs,
        readLogsBytes,
      });
