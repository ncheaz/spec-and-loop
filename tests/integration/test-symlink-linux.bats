...
# Integration test for symlink creation on Linux
# Tests that symlink creation works correctly on Linux platform
...
@test "symlink on Linux: symlink points to absolute path" {
  create_git_repo
...
@test "symlink on Linux: writing to symlink updates target file" {
  create_git_repo
...
@test "symlink on Linux: reading symlink returns target file content" {
  create_git_repo
...
@test "symlink on Linux: symlink is created in correct directory" {
  create_git_repo
...
@test "symlink on Linux: replacing existing symlink updates target" {
  create_git_repo
...
@test "symlink on Linux: symlink persists after script completion" {
  create_git_repo
...
@test "symlink on Linux: symlink creation is idempotent" {
  create_git_repo
